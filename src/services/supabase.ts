...
export const authService = {
     ...
     */
    async checkEmailConfirmed(email: string) {
        try {
            // Get the user by email (requires admin or special setup)
            // For regular clients, we check via signIn which will fail if not confirmed
            // This is a workaround - we'll check on login attempt
    ...
    /**
     * Update user metadata with vault salt and master password hash
     */
    async updateUserMetadata(userId: string, metadata: { vault_salt?: string; master_password_hash?: string }) {
        try {
